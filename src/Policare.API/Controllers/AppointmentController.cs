...
public class AppointmentsController : ControllerBase
{
    ...
    // <summary>
    /// 🔧 DEBUG: Analizza perché non si trovano slot ottimali
    /// </summary>
    [HttpPost("debug-scheduling")]
    public async Task<ActionResult<SchedulingDebugInfo>> DebugScheduling([FromBody] FindOptimalSlotRequest request)
    {
        try
        {
            var debugInfo = new SchedulingDebugInfo
            {
            ...
            // 1. Verifica medici disponibili
            var doctors = await _schedulingService.GetAvailableDoctorsAsync(request.ClinicId, request.PreferredDoctorId, request.AppointmentType);
            debugInfo.AvailableDoctors = doctors.Count();
            debugInfo.DoctorIds = doctors.Select(d => d.Id).ToList();

            // 2. Verifica sale disponibili
            var rooms = await _schedulingService.GetAvailableRoomsAsync(request.ClinicId);
            debugInfo.AvailableRooms = rooms.Count();
            debugInfo.RoomIds = rooms.Select(r => r.Id).ToList();

            // 3. Verifica finestra di ricerca
            var searchWindow = GenerateSearchWindow(request.PreferredDate, request.Preferences);
            debugInfo.SearchStartDate = searchWindow.StartDate;
            debugInfo.SearchEndDate = searchWindow.EndDate;

            // 4. Verifica appuntamenti esistenti
            var existingAppointments = await _appointmentService.GetAppointmentsByClinicAsync(request.ClinicId);
            var appointmentsInWindow = existingAppointments.Where(a =>
                a.StartTime >= searchWindow.StartDate && a.StartTime <= searchWindow.EndDate);
            debugInfo.ExistingAppointmentsInWindow = appointmentsInWindow.Count();

            // 5. Genera alcuni slot di esempio per debug
            if (doctors.Any() && rooms.Any())
            {
                var firstDoctor = doctors.First();
                var firstRoom = rooms.First();

                var sampleSlots = await FindSlotsForDoctorRoomAsync(
                    firstDoctor, firstRoom, searchWindow.StartDate, searchWindow.EndDate, request.DurationMinutes);

                debugInfo.SampleSlotsGenerated = sampleSlots.Count();
                debugInfo.SampleSlots = sampleSlots.Take(5).Select(s => new SlotDebugInfo
                {
        ...
        {
            _logger.LogError(ex, "Error in debug scheduling");
            return StatusCode(500, ex.Message);
        }
    }

    // Metodi helper che dobbiamo rendere pubblici o spostare nel servizio
    private (DateTime StartDate, DateTime EndDate) GenerateSearchWindow(DateTime preferredDate, SmartSchedulingPreferencesDto? preferences)
    {
